...
| 12 | In-browser reader | done #36 | epub.js over the whole file from `GET /books/{id}/file`, contents drawer, three text sizes, scroll position written back as progress. Full takeover outside `AppShell`, and a `BookReader` seam because epub.js cannot run in jsdom — designed in [reader-screen.md](reader-screen.md) |
